...
📊 **Interactive Dashboard**: Built with Streamlit to manage, trigger, and monitor test executions without touching the command line.

🗄️ **Persistence Layer**: Integrated SQLite DB to store test scenarios, history, and detailed execution metrics.

🐳 **Containerization**: Fully Dockerized setup for consistent execution across different environments.

⚡ **One-Click Startup**: A custom start_project.bat for Windows users that handles virtual environment (venv) creation and dependency installation automatically.

🛠️ **CI/CD Ready**: Integrated with Jenkins (via Docker) for automated pipeline triggers.

📑 **Advanced Reporting**:

Allure Reports: Rich HTML reports with step-by-step visibility.

Video Recording: Automatic capture of test executions, attached directly to Allure reports.

Detailed Logging: Rotating file logs for deep-dive debugging.
## ✨ Framework Features
...
## 🚀 Getting Started
### With Docker setup
* Ensure you have Docker desktop setup in local machine.
* Clone the repo and place .env file in the root directory.
* Run below command to execute tests through docker container.
* ```bash
    docker compose up --build
    ```
* Once container is up, you need to navigate to http://localhost:8501/ to view the dashboard
* Run below command to close the docker container
* ```bash
    docker compose down
    ```
### Without Docker setup (local)
...
5. You can click Start allure server button to generate allure report.
6. Click the link once the server gets started to see the allure visual report
...
| `dockerfile`                             | Instructions for building the Linux-based automation image.                                                                                                   |
